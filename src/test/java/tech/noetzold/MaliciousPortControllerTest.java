...
public class MaliciousPortControllerTest {
    ...
    public void testGetAllMaliciousPorts() {
        given()
                .auth().oauth2(accessToken)
                .when()
                .get("http://localhost:9000/port?page=1&size=10&sortBy=id")
                .then()
                .statusCode(200);
    }
    ...
    @Order(2)
    @TestSecurity(user = "admin", roles = {"admin"})
    public void testGetMaliciousPortById() {
        given()
                .auth().oauth2(accessToken)
                .when()
    ...
    @Order(3)
    @TestSecurity(user = "admin", roles = {"admin"})
    public void testSaveMaliciousPort() {
        final JsonObject json = Json.createObjectBuilder()
                .add("vulnarableBanners", "Test Banner")
                .build();
    ...
    @Order(4)
    @TestSecurity(user = "admin", roles = {"admin"})
    public void testRemoveMaliciousPort() {
        given()
                .auth().oauth2(accessToken)
                .when()
                .delete("http://localhost:9000/port/1")
                .then()
                .statusCode(202);
    }
}
