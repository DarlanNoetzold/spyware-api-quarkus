...
public class MaliciousProcessControllerTest {
    ...
    public void testGetAllMaliciousProcesses() {
        given()
                .auth().oauth2(accessToken)
                .when()
                .get("http://localhost:9000/process?page=1&size=10&sortBy=id")
                .then()
                .statusCode(200);
    }
    ...
    @Order(2)
    @TestSecurity(user = "admin", roles = {"admin"})
    public void testGetMaliciousProcessById() {
        given()
                .auth().oauth2(accessToken)
                .when()
                .get("http://localhost:9000/process/1")
                .then()
                .statusCode(404);
    }
    ...
    @Order(3)
    @TestSecurity(user = "admin", roles = {"admin"})
    public void testSaveMaliciousProcess() {
        final JsonObject json = Json.createObjectBuilder()
                .add("nameExe", "Test Process")
                .build();
    ...
    @Order(4)
    @TestSecurity(user = "admin", roles = {"admin"})
    public void testRemoveMaliciousProcess() {
        given()
                .auth().oauth2(accessToken)
                .when()
                .delete("http://localhost:9000/process/1")
                .then()
                .statusCode(202);
    }
}
